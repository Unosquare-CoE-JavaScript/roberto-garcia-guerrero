import React, { useCallback, useEffect, useReducer, useLayoutEffect, useRef, useState } from "react";
...
/**
 * custom hook for getting previoys prop/state
 */
...
// first invocation takes in the function params
// second invocation takes in whatever arguments are passed in,
// loops over the function list and onvokes the functions with the arguments
  ...
  return (
    <div>
      ...
      <section>
        ...
        <pre className={userStyles.resetMsg}>{JSON.stringify({ timesClapped, count, countTotal })}</pre>
        <pre className={userStyles.resetMsg}>{uploadingReset ? `uploading reset ${resetDep}...` : ""}</pre>
        <pre style={{ color: "red" }}>{isClappedTooMuch ? "you have clapped too muc, dont be so generous" : ""}</pre>
      </section>
    </div>
  );
};
