...
const MedidumClapContext = createContext();
const { Provider } = MedidumClapContext;
...
const ClapIcon = ({ style: userStyles = {}, className }) => {
  const { isClicked } = useContext(MedidumClapContext);
  return (
    <span>
      <svg
        id="clapIcon"
...
const ClapCount = ({ style: userStyles = {}, className }) => {
  const { count, setRef } = useContext(MedidumClapContext);

  return (
    <span ref={setRef} data-refkey="clapCountRef" className={`${styles.count} ${className}`} style={userStyles}>
      + {count}
    </span>
  );
};
...
const CountTotal = ({ style: userStyles = {} }) => {
  const { countTotal, setRef } = useContext(MedidumClapContext);
...
// to avoid importing { ClapIcon, ClapCount, CountTotal } we add the properties
const Usage = () => {
  ...
  return (
    <div style={{ width: "100%" }}>
      {/* <MediumClap onClap={handleClap} style={{ border: "2px solid red " }}> */}
